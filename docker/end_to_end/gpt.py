...
from kernels_attn import flash_attention_v2
from kernels_ffn import fused_ffn_txl
...
class FusedAttention_v2(nn.Module):
...
class FusedAttention_v1(nn.Module):
    def __init__(self, hidden_size, num_heads, dropout_prob=0.0):
        super().__init__()
        self.dropout_prob = dropout_prob
        self.num_heads = num_heads
...
class FusedMLP_triton(nn.Module):
    def __init__(self, hidden_size, dropout_prob=0.0):
        super().__init__()
...
class FusedGPT(nn.Module):
    def __init__(self, config, txl=False):
        super().__init__()
        self.config = config
        ...
        FusedAttention = FusedAttention_v2 if txl else FusedAttention_v1
        FusedMLP = FusedMLP_txl if txl else FusedMLP_triton
...
def convert_hf_and_load_model(model_id, device):
    ...
    txl_model = FusedGPT(config, txl=True)
    model.load_state_dict(state_dict)
    txl_model.load_state_dict(state_dict)
    print("Model loaded.")
    return model.to(device).eval(), hf_model.to(device).eval(), txl_model.to(device).eval()

def load_model_only(device):
    config = GPTConfig(
        vocab_size=50304,
        block_size=16384,
        n_layer=12,
    ...
    model = FusedGPT(config)
    print("Model created.")
    return model.to(device).eval(), txl_model.to(device).eval()
