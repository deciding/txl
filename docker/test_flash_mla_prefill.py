...
def run_test(p: TestParam) -> bool:
    ...
    if p.check_correctness:
        ...
        is_correct &= check_is_allclose("out_txl", ans_out_txl, ref_out, abs_tol=8e-4, rel_tol=2.01 / 128, cos_diff_tol=7e-6)
        is_correct &= check_is_allclose("max_logits_txl", ans_max_logits_txl, ref_max_logits, abs_tol=1e-6, rel_tol=2.01 / 65536)
