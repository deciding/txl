...
@torch.inference_mode()
def run_txl_mla(q, block_table, blocked_k, max_seqlen_pad, block_size, b, s_q, cache_seqlens,
                    h_q, h_kv, d, dv, causal, dtype):
    from flash_mla import make_mla_runner

    q_value = q[..., :dv].permute(0, 2, 1, 3).contiguous()
    q_pe = q[..., dv:].permute(0, 2, 1, 3).contiguous()
    blocked_k = blocked_k.view(b, max_seqlen_pad, h_kv, d)
    k_value = blocked_k[..., :dv].permute(0, 2, 1, 3).contiguous()
    k_pe = blocked_k[..., dv:].permute(0, 2, 1, 3).contiguous()
    # print(f"{q_value.shape=}, {q_pe.shape=}, {k_value.shape=}, {k_pe.shape=}")
...
def compare_a(target, b, s_q, cache_seqlens, h_q, h_kv, d, dv, causal, dtype):
    ...
    max_seqlen = cache_seqlens.max().item()
    max_seqlen_pad = triton.cdiv(max_seqlen, 256) * 256
    mean_seqlens = cache_seqlens.float().mean().item()
    # print(f"{total_seqlens=}, {mean_seqlens=}, {max_seqlen=}")
...
shape_configs = [{
    ...
    "dtype":
        torch.float16
} for batch in [132] for seqlen in [1024, 2048, 4096, 8192, 16384, 32768] for head in [128]]
