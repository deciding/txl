...
tma_cfg = dict(BLOCK_M=64, BLOCK_N=64, NUM_STAGES=2)

@txl.autotune(
  configs=[txl.Config(tma_cfg, num_stages=2, num_warps=4, num_warpgroups=1, pre_hook=_pre_hook)],
...
# 需要调整
ws_cfg = dict(BLOCK_M=128, BLOCK_N=64, NUM_STAGES=2)

@txl.autotune(
    configs=[txl.Config(ws_cfg, num_stages=2, num_warps=4, num_warpgroups=3, pre_hook=_pre_hook_divm)],
...
def test_op(Z, H, N_Q, KV_HEADS, KV_SEQ_LEN, R0, PE_DIM, dtype=torch.float16, algo=0, no_tune=False):
    ...
    # kvu = (torch.ones((Z, KV_HEADS, KV_SEQ_LEN, R0//2), dtype=dtype, device=DEVICE)*0.5)
    # kvd = (torch.ones((Z, KV_HEADS, KV_SEQ_LEN, R0//2), dtype=dtype, device=DEVICE)*2.0)
    # kv = torch.cat([kvu, kvd], dim=3)
...
if __name__ == "__main__":
    ...
    # test_op(1, 1, 64, 1, 256, 128, 64, algo=6, no_tune=no_tune)
    test_op(16, 32, 1024, 1, 1024, 512, 64, algo=6, no_tune=no_tune)
