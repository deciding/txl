...
// Comparison function for sorting Channel pointers
struct ChannelSorter {
  bool operator()(Channel *a, Channel *b) const {
    // First compare by producer ID (first element of relation)
    if (a->relation.first != b->relation.first) {
      return a->relation.first < b->relation.first;
    }
    
    // If same producer, compare by consumers (second element of relation)
    const auto &consumersA = a->relation.second;
    const auto &consumersB = b->relation.second;
    
    // Compare consumer vectors lexicographically
    if (!std::equal(consumersA.begin(), consumersA.end(), 
                   consumersB.begin(), consumersB.end())) {
      return std::lexicographical_compare(consumersA.begin(), consumersA.end(),
                                         consumersB.begin(), consumersB.end());
    }
    
    // If same consumers, compare by operand index
    if (a->operandIdx != b->operandIdx) {
      return a->operandIdx < b->operandIdx;
    }
    
    // As last resort, compare by operation pointer address
    // (this provides a total order even if all other fields are equal)
    return a->op < b->op;
  }
};
...
void getTransitiveUsers(Value root,
  ...
  for (Operation *userOp : root.getUsers()) {
    ...
    else if (auto forOp = dyn_cast<scf::ForOp>(userOp)) {
      unsigned operandIndex = 0;
      for (OpOperand &operand : forOp->getOpOperands()) {
        if (operand.get() == root) {
          unsigned argIdx = 0;
          if (operandIndex < forOp.getNumControlOperands())
              argIdx = 0;
          else
              argIdx = operandIndex - (forOp.getNumControlOperands() - 1);
          auto result = forOp.getBody()->getArgument(argIdx);
          getTransitiveUsers(result, users);
        }
        operandIndex++;
      }
...
DenseMap<Channel *, Value> createBuffer(
  ...
  SmallVector<Channel*> sortedProducers;
  for (auto &pair : channelsGroupedByProducers) {
    sortedProducers.push_back(pair.first);
  }
  llvm::sort(sortedProducers, ChannelSorter());
  ...
  for (auto *producer : sortedProducers) {
    //auto &channels = item.second;
    auto &channels = channelsGroupedByProducers[producer];
    for (auto c : channels) {
  ...
  for (auto *producer : sortedProducers) {
    ...
    auto &channels = channelsGroupedByProducers[producer];
    auto srcValue = producer->getSrcOperand();
    auto srcOp = producer->getSrcOp();
    unsigned numBuffers = channels.front()->numBuffers;
...
void insertAsyncComm(
  ...
  auto consumerReleaseHeuristic = [&](Operation *p, Operation *c,
                                      int consumerAsyncTaskId,
                                      SmallVector<AsyncTaskId> allConsumerAsyncTaskIds) -> Operation * {
    ...
    for (auto &op : reverse(c->getBlock()->getOperations())) {
      ...
      if (asyncTasks.size() == 1 && asyncTasks[0] == consumerAsyncTaskId)
        return &op;
      else if (asyncTasks.size() == allConsumerAsyncTaskIds.size() &&
              std::is_permutation(asyncTasks.begin(), asyncTasks.end(), allConsumerAsyncTaskIds.begin())){
        return &op;
      }
    }
  ...
  for (auto kv : channelsGroupedByConsumers) {
    ...
    SmallVector<AsyncTaskId> allConsumerAsyncTaskIds;
    for (auto token : tokens) {
      allConsumerAsyncTaskIds.push_back(token.first);
...
public:
  ...
  void runOnFuncOp(triton::FuncOp funcOp) {
    ...
    LLVM_DEBUG({
      LDBG("\n\nAfter insertAsyncCopy");
      funcOp.dump();
      LDBG("\n\nDone insertAsyncCopy");
    });
    ...
    LLVM_DEBUG({
      LDBG("\n\nAfter insertAsyncComm");
      funcOp.dump();
      LDBG("\n\nDone insertAsyncComm");
    });
    ...
    LLVM_DEBUG({
      LDBG("\n\nAfter SpecializeRegion");
      funcOp.dump();
      LDBG("\n\nDone SpecializeRegion");
    });
  }
