...
public:
  ...
  virtual void barrierWithId(Location loc, RewriterBase &rewriter,
                       bool isWarpSync = false, SmallVector<int> wgIds={}, int numThreads = 128) const {};
  ...
  virtual void storeDShared(RewriterBase &rewriter, Location loc, Value ptr,
                            ...
                            Value pred) const = 0;
  virtual void storeDSharedMbar(RewriterBase &rewriter, Location loc, Value ptr,
                            std::optional<Value> ctaId, Value val,
                            Value pred, Value mbarPtr=Value()) const {};
