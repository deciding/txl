...
// txl
class AsyncLoadOpAxisInfoVisitor final : public AxisInfoVisitorImpl<triton::AsyncLoadOp> {
public:
  using AxisInfoVisitorImpl<triton::AsyncLoadOp>::AxisInfoVisitorImpl;

  AxisInfo
  getAxisInfo(triton::AsyncLoadOp op,
              ArrayRef<const dataflow::Lattice<AxisInfo> *> operands) override {
    // If pointers and mask both have constancy properties, those properties
    // will also extend to output.
    AxisInfo ptrInfo = operands[1]->getValue();
    std::optional<AxisInfo> maskInfo;
    if (operands.size() > 2) {
      maskInfo = operands[2]->getValue();
    }
    AxisInfo::DimVectorT contiguity;
    AxisInfo::DimVectorT divisibility;
    AxisInfo::DimVectorT constancy;
