...
SmallVector<Value> lowerLdStPred(
    ...
    std::function<SmallVector<Value>(RewriterBase &, Location, ArrayRef<Value>,
                                     Value, int, VectorType, Value)> // txl pred
        lowerInst) {
  ...
  int threadsPerWarp = triton::gpu::lookupThreadsPerWarp(rewriter);
  int numWarps = triton::gpu::lookupNumWarps(lookupPt);
  int subThreadsPerWarp = threadsPerWarp;
  int subNumWarps = numWarps;
  Value pred = b.true_val();
  if (i8AddrLayout.hasInDim(kLane)){
      subThreadsPerWarp = i8AddrLayout.getInDimSize(kLane);
      if (subThreadsPerWarp < threadsPerWarp) {
          pred = b.and_(b.icmp_ult(laneId, b.i32_val(subThreadsPerWarp)), pred);
      }
  }
  if (i8AddrLayout.hasInDim(kWarp)){
      subNumWarps = i8AddrLayout.getInDimSize(kWarp);
      if (subNumWarps < numWarps) {
          pred = b.and_(b.icmp_ult(warpId, b.i32_val(subNumWarps)), pred);
      }
  }
  ...
  for (int i = 0; i < cvt.getInDimSize(kReg); i += nAdditive) {
    ...
    for (int j = 0; j < nAdditive; j += elemsPerVec) {
      ...
      llvm::append_range(outVals,
                         lowerInst(rewriter, loc, vals, vecAddr, i + j, vecTy, pred));
    }
  }
