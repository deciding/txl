...
static Value mapa(RewriterBase &rewriter, Location loc, Value ptr, Value ctaid,
  ...
  PTXBuilder builder;
  std::string in32Constraint = "r";
  std::string out32Constraint = "=r";
  auto *outOpr = builder.newOperand(out32Constraint);
  auto *ptrOpr = builder.newOperand(ptr, "r");
  auto *ctaIdOpr = builder.newOperand(ctaid, in32Constraint);
  auto mapa1 = builder.create<>("mapa")
              ->o("shared::cluster")
              .o("u32"); // TODO: u32 or u64
  mapa1(outOpr, ptrOpr, ctaIdOpr);
  Type resultTy = Type(ptr_ty(rewriter.getContext(), 7));
  ptr = builder.launch(rewriter, loc, resultTy, /*hasSideEffect=*/ true);
  return ptr;
}
