...
// txl
int getAsyncTaskId(Operation* op) {
   if (auto taskIdAttr = op->getAttrOfType<mlir::IntegerAttr>("ttxg.wgid")) {
       int asyncTaskId = taskIdAttr.getInt();
       return asyncTaskId;
   }
   return -1;
}
LogicalResult replaceProtonCtxOp(OpBuilder &builder, FuncOp func,
                                    Value buffer, Value profileMem,
                                    Value segment, MetricType metricType,
                                    ModuleScopeIdAllocation &scopeInfo,
                                    bool clockExtension) {
  mlir::IntegerType clkType =
  ...
  func->walk([&](ttxg::InitCtxOp initCtxOp) {
    auto loc = initCtxOp.getLoc();
    builder.setInsertionPoint(initCtxOp);

    builder.create<gpu::InitCtxOp>(loc, profileMem);
    initCtxOp.erase();
  });

  llvm::DenseMap<int, Value> asyncTaskIdToSegment;
  // Replace all proton::RecordOp in the worker warps.
  func->walk([&](ttxg::RestoreCtxOp restoreCtxOp) {
  ...
  func->walk([&](ttxg::SaveCtxOp saveCtxOp) {
...
public:
  ...
  LogicalResult circularRecordStrategyLowering(FuncOp func) {
    ...
    // txl
    if (failed(replaceProtonCtxOp(builder, func,
                                     buffer, profileMem,
                                     segment, metricType,
