...
public:
  ...
  Operation *rewriteLoadStoreOp(OpBuilder &builder, Operation *op,
    ...
    if (auto loadOp = dyn_cast<triton::LoadOp>(op))
      ...
      newOther = info.generateOther(builder, op->getLoc(), optPadding);
    }
    ...
    if (auto loadOp = dyn_cast<triton::LoadOp>(op)) {
      ...
      auto newResult = builder.create<triton::AsyncLoadOp>(
          ...
          loadOp.getSrc(), newPtr, newMask, newOther, 
          /*boundaryCheck=*/builder.getDenseI32ArrayAttr(ArrayRef<int32_t>{}), /*padding=*/triton::PaddingOptionXAttr(),
          loadOp.getCache(),
          loadOp.getEvict(), loadOp.getIsVolatile());
      op->getResult(0).replaceAllUsesWith(newResult);
      if (op->getAttr("async_task_id"))
