...
void getBackwardSliceToPartition(Value root, unsigned dim, int sliceSize,
                                 ...
                                 SetVector<Operation *> &ignoredSlice) {
    ...
    if (auto op = v.getDefiningOp()) {
      ...
      if (backwardSlice.insert(op)) {
        if (op->hasTrait<OpTrait::Elementwise>() ||
          ...
          if (dim == 0){
            if (auto opB = dotOp.getB().getDefiningOp()) {
              ignoredSlice.insert(opB);
            }
          }
          else{
            if (auto opA = dotOp.getA().getDefiningOp()) {
              ignoredSlice.insert(opA);
            }
          }
...
void getBackwardSliceToIgnore(Value root, unsigned dim, int sliceSize,
                                 SetVector<Operation *> &ignoredSlice) {
  SmallVector<Value> queue = {root};
  while (!queue.empty()) {
    auto v = queue.back();
    queue.pop_back();
    if (auto op = v.getDefiningOp()) {
      if (ignoredSlice.insert(op)) {
        if (op->hasTrait<OpTrait::Elementwise>() ||
            isa<arith::ConstantOp, arith::ExtSIOp, arith::ExtUIOp,
                arith::ExtFOp, BroadcastOp, ExpandDimsOp, MakeRangeOp, SplatOp,
                ConvertLayoutOp, triton::gpu::LocalAllocOp, LoadOp,
                ExperimentalDescriptorLoadOp, nvidia_gpu::TMEMAllocOp,
                nvidia_gpu::TMEMLoadOp, MemDescTransOp>(op)) {
          for (Value operand : op->getOperands())
            queue.push_back(operand);
        } else if (auto tensorDescOp = dyn_cast<ReinterpretTensorDescOp>(op)) {
          continue;
          ...
          auto thenYieldArg = ifOp.thenYield().getOperand(resultIndex);
          ignoredSlice.insert(ifOp.thenYield());
          queue.push_back(thenYieldArg);
          auto elseYieldArg = ifOp.elseYield().getOperand(resultIndex);
          ignoredSlice.insert(ifOp.elseYield());
          queue.push_back(elseYieldArg);
        } else {
          continue;
        }
      ...
      if (auto forOp = dyn_cast<scf::ForOp>(bbAargOwner)) {
        LLVM_DEBUG({
          LDBG("START INDEX");
          LDBG(bbArg.getArgNumber());
          bbArg.dump();
          forOp->dump();
        });
        // track initial value
        if (bbArg.getArgNumber() >= forOp.getNumInductionVars()) {
            auto initArg = forOp.getInitArgs()[bbArg.getArgNumber() - forOp.getNumInductionVars()];
            queue.push_back(initArg);
            // track yield value
            auto yieldArg = forOp.getYieldedValues()[bbArg.getArgNumber() - forOp.getNumInductionVars()];
            queue.push_back(yieldArg);
        }
      }
    }
...
static void
 getForwardSliceStrictImpl(Operation *op, SetVector<Operation *> *forwardSlice,
     ...
     for (Block &block : region){
       for (Operation &blockOp : block)
         if (forwardSlice->count(&blockOp) == 0){
           getForwardSliceStrictImpl(&blockOp, forwardSlice, filter);
         }
     }
   for (Value result : op->getResults()) {
     for (Operation *userOp : result.getUsers())
       if (forwardSlice->count(userOp) == 0)
         getForwardSliceStrictImpl(userOp, forwardSlice, filter);
   }
  
   forwardSlice->insert(op);
 }
  
 void getForwardSliceStrict(Operation *op, SetVector<Operation *> *forwardSlice,
                            const ForwardSliceOptions &options={}) {
   getForwardSliceStrictImpl(op, forwardSlice, options.filter);
 ...
 void getForwardSliceStrict(Value root, SetVector<Operation *> *forwardSlice,
                            const SliceOptions &options={}) {
   for (Operation *user : root.getUsers())
     getForwardSliceStrictImpl(user, forwardSlice, options.filter);
...
void getForwardSliceToPartition(Value root, unsigned dim, int sliceSize,
  ...
  SliceOptions sliceOptions([&ignoredSlice](Operation* op) {
    return !ignoredSlice.contains(op) && !dyn_cast<ReduceReturnOp>(op);
  });
  while (!queue.empty()) {
    auto v = queue.back();
    queue.pop_back();
    ...
    getForwardSliceStrict(v, &forwardSlice, sliceOptions);
...
void getSliceToPartition(Value root, unsigned dim, int sliceSize,
                         SetVector<Operation *> &slice, SetVector<Operation *> &ignoredSlice) {
  getBackwardSliceToPartition(root, dim, sliceSize, slice, ignoredSlice);

  SetVector<Operation *> forwardSlice;
  getForwardSliceToPartition(root, dim, sliceSize, forwardSlice, ignoredSlice);
  slice.insert(forwardSlice.begin(), forwardSlice.end());
  for (auto op : forwardSlice) {
    if (op->hasTrait<OpTrait::Elementwise>() ||
        isa<tt::StoreOp, ExperimentalDescriptorStoreOp>(op)) {
      for (OpOperand &operand : op->getOpOperands()) {
        getBackwardSliceToPartition(operand.get(), dim, sliceSize, slice, ignoredSlice);
      }
    } else if (auto dotOp = dyn_cast<nvidia_gpu::WarpGroupDotOp>(op)) {
      getBackwardSliceToPartition(dim == 0 ? dotOp.getA() : dotOp.getB(), dim,
                                  sliceSize, slice, ignoredSlice);
      if (dim == 0){
        if (auto opB = dotOp.getB().getDefiningOp()) {
          ignoredSlice.insert(opB);
        }
      ...
      getBackwardSliceToPartition(dotOp.getC(), dim, sliceSize, slice, ignoredSlice);
    } else if (auto dotOp = dyn_cast<nvidia_gpu::TCGen5MMAOp>(op)) {
      getBackwardSliceToPartition(dim == 0 ? dotOp.getA() : dotOp.getB(), dim,
                                  sliceSize, slice, ignoredSlice);
      if (dim == 0){
      ...
      getBackwardSliceToPartition(dotOp.getD(), dim, sliceSize, slice, ignoredSlice);
    }
  }
}
...
bool computePartitionScheme(triton::FuncOp &funcOp,
  ...
  SmallVector<int> partitionSizes;
  SetVector<Operation *> ignoredSlice;
  ...
  for (auto op : dots) {
    ...
    getBackwardSliceToIgnore(ignoredOperand, partitionDim, partitionSize, ignoredSlice);
  }
  ...
  for (int i = 0; i < accumulators.size(); i++){
    ...
    getSliceToPartition(accumulator, partitionScheme.partitionDim, partitionSize, slice, ignoredSlice);
  ...
  auto cloneAndSetResultType = [&](Operation *op) {
    ...
    if (!op->getResults().empty()) {
      ...
      if (auto type = dyn_cast<MemDescType>(v.getType())) {
        ...
        SmallVector<int64_t> shape{type.getShape().begin(),
                                   type.getShape().end()};
        // We need to handle the case where the dim is not expanded yet.
        // if dim == 1, we simply assume this is unepxanded, and currently we still
        // clone it
        int sliceSize = 1;
        if (shape[dim] != 1)
            sliceSize = shape[dim] / numOfPartitions;
        shape[dim] = sliceSize;
  ...
  if (op->hasTrait<OpTrait::Elementwise>() ||
    ...
    if (auto loadOp = dyn_cast<ExperimentalDescriptorLoadOp>(op)) {
      ...
      sliceOp(storeOp.getDesc(), offset, mappings, reverseMappings,
              partitionScheme);
      // src op is missing for store op
      sliceOp(storeOp.getSrc(), offset, mappings, reverseMappings,
              partitionScheme);
    ...
    if (offset) {
      ...
      // do not add the coord val to mapping to pollute all the other vals
      newIndices[dim] = newCoordVal;
    }

    newOp = cloneAndSetResultType(op);
    if (auto loadOp = dyn_cast<ExperimentalDescriptorLoadOp>(newOp)){
        loadOp.getIndicesMutable().assign(newIndices);
    }
    else if (auto storeOp = dyn_cast<ExperimentalDescriptorStoreOp>(newOp)){
    ...
    for (unsigned i = 0; i < forOp.getInitArgs().size(); i++) {
      ...
      if (isa<scf::ForOp>(initArgOp) && mappings.contains(initArg)){
        newInitArg = mappings.lookupOrNull(initArg);
      }
      else if (auto bbArg = dyn_cast<BlockArgument>(initArg)) {
...
void doDeepCleanup(triton::FuncOp &funcOp,
  ...
  do {
    ...
    funcOp.walk([&](Operation *op) {
      ...
      bool notUsed = true;
      if (auto reduceReturnOp = dyn_cast<ReduceReturnOp>(op)){
        op = reduceReturnOp->getParentOp();
      }
      ...
      if (notUsed && !opsToDelete.contains(op))
