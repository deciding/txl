...
void LayoutPropagation::resolveConflicts() {
  for (auto &it : layouts) {
    ...
    bool isLoadOrStore =
        op && isa<LoadOp, AsyncLoadOp, StoreOp, AtomicRMWOp, AtomicCASOp>(op); // txl
...
void LayoutRematerialization::backwardRematerialization(
  ...
  for (Operation *op : sliceOps) {
    ...
    if (isOpSingleUse(op)) {
      ...
      continue;
    } else if (isa<LoadOp>(op) || isa<AsyncLoadOp>(op) || isa<LocalLoadOp>(op)) { // txl
...
void LayoutRematerialization::hoistConvertDotOperand(
  ...
  for (Value v : slice) {
    ...
    if (!isa<LoadOp, AsyncLoadOp, DescriptorLoadOp>(v.getDefiningOp())) { // txl
