...
from triton.language import constexpr, str_to_ty, tensor, tuple as tl_tuple
from triton.language.core import _unwrap_if_constexpr, base_value, base_type
from triton.runtime.jit import get_jit_fn_file_line, get_full_name, JITCallable, BoundConstexprFunction, ConstexprFunction, JITFunction
...
class CodeGenerator(ast.NodeVisitor):
    ...
    def __init__(self, context, prototype, gscope, function_name, jit_fn: JITFunction, *, options, codegen_fns, module_map, is_gluon, module=None, is_kernel=False, function_types: Optional[Dict]=None, noinline=False, caller_context=None, file_name: Optional[str]=None, begin_line=0):
    ...
    builtin_namespace.update((('print', language.core.device_print), ('min', language.minimum), ('max', language.maximum)))
    ...
    def _define_name_lookup(self):
        ...
        def global_lookup(name: str, absent):
            ...
            if any([val is absent, name in self.builtin_namespace, type(val) is ModuleType, isinstance(val, JITCallable), getattr(val, '__triton_builtin__', False), getattr(val, '__triton_aggregate__', False), getattr(val, '__module__', '').startswith('triton.language'), getattr(val, '__module__', '').startswith('triton.experimental.gluon.language'), isinstance(val, language.dtype), _is_namedtuple(val), self._is_constexpr_global(name), self.visiting_arg_default_value, knobs.compilation.allow_non_constexpr_globals]):
                return val
            raise NameError(textwrap.dedent(f"                Cannot access global variable {name} from within @jit'ed\n                function. Triton kernels can only access global variables that\n                are instanstiated as constexpr (`x = triton.language.constexpr(42)`). Note that this is different from\n                annotating a variable as constexpr (`x: triton.language.constexpr = 42`), which is not supported.  Alternatively, set the\n                envvar TRITON_ALLOW_NON_CONSTEXPR_GLOBALS=1, but we do not\n                promise to support this forever.").replace('\n', ' '))
        absent_marker = object()
    ...
    def visit_BinOp(self, node):
        ...
        if method_name is None:
            raise self._unsupported(node, "AST binary operator '{}' is not (currently) implemented.".format(node.op.__name__))
        return self._apply_binary_method(method_name, lhs, rhs)
    _method_name_for_bin_op: Dict[Type[ast.operator], str] = {ast.Add: '__add__', ast.Sub: '__sub__', ast.Mult: '__mul__', ast.Div: '__truediv__', ast.FloorDiv: '__floordiv__', ast.Mod: '__mod__', ast.Pow: '__pow__', ast.LShift: '__lshift__', ast.RShift: '__rshift__', ast.BitAnd: '__and__', ast.BitOr: '__or__', ast.BitXor: '__xor__'}
    ...
    def visit_then_else_blocks(self, node, liveins, then_block, else_block):
        ...
        for name, value in liveins.items():
            ...
            for defs, block_name in [(then_defs, 'then'), (else_defs, 'else')]:
                type_equal = type(defs[name]) == type(value)
                assert type_equal and defs[name].type == value.type, f'initial value for `{name}` is of type {value}, but the {block_name} block redefines it as {defs[name]}'
        for name in sorted(then_defs.keys() & else_defs.keys()):
            ...
            type_equal = type(then_val) == type(else_val)
            assert type_equal and then_ty == else_ty, f'Mismatched type for {name} between then block ({then_ty}) and else block ({else_ty})'
            names.append(name)
    ...
    def visit_if_top_level(self, cond, node):
        with enter_sub_region(self) as sr:
            ...
            then_defs, else_defs, then_block, else_block, names = self.visit_then_else_blocks(node, liveins, then_block, else_block)
    ...
    def visit_If(self, node):
        ...
        if _is_triton_tensor(cond):
            ...
            if cond.type.is_block():
                warnings.warn('If conditional called with multidimensional Tensor instead of scalar; please use "if (%s).item()" instead' % ast.unparse(node.test))
            ...
            if ContainsReturnChecker(self.gscope).visit(node):
                if self.scf_stack:
                    raise self._unsupported(node, 'Cannot have `return` statements inside `while` or `for` statements in triton.')
            ...
            if type(cond) not in _condition_types:
                raise self._unsupported(node, '`if` conditionals can only accept values of type {{{}}}, not objects of type {}'.format(', '.join((_.__name__ for _ in _condition_types)), type(cond).__name__))
    ...
    def visit_Compare(self, node):
        ...
        if method_name is None:
            raise self._unsupported(node, "AST comparison operator '{}' is not (currently) implemented.".format(node.ops[0].__name__))
        return self._apply_binary_method(method_name, lhs, rhs)
    _method_name_for_comp_op: Dict[Type[ast.cmpop], str] = {ast.Eq: '__eq__', ast.NotEq: '__ne__', ast.Lt: '__lt__', ast.LtE: '__le__', ast.Gt: '__gt__', ast.GtE: '__ge__'}
    ...
    def visit_UnaryOp(self, node):
        ...
        try:
            ...
            raise self._unsupported(node, f"AST unary operator '{fn}' is not (currently) implemented on type {type(operand).__name__}")
    ...
    def _verify_loop_carried_variable(self, name, loop_val, live_val):
        ...
        assert type(loop_val) is type(live_val), f'Loop carried variable {name} changed type, was {type(loop_val)} but is now {type(live_val)}'
        assert not _is_triton_tensor(loop_val) or loop_val.type == live_val.type, f'Loop-carried variable {name} has initial type {live_val.type} but is re-assigned to {loop_val.type} in loop! Please make sure that the type stays consistent.'
    ...
    def call_JitFunction(self, fn: JITFunction, args, kwargs, caller_context=None):
        ...
        if not self.module.has_function(fn_name):
            ...
            generator = CodeGenerator(self.context, prototype, fn.get_capture_scope(), module=self.module, jit_fn=fn, function_name=fn_name, function_types=self.function_ret_types, noinline=fn.noinline, file_name=file_name, begin_line=begin_line, options=self.builder.options, codegen_fns=self.builder.codegen_fns, module_map=self.builder.module_map, caller_context=caller_context, is_gluon=self.is_gluon)
            try:
    ...
    def call_Function(self, node, fn, args, kws):
        ...
        if hasattr(fn, '__self__') and _is_triton_value(fn.__self__) or language.core.is_builtin(fn) or isinstance(fn, ConstexprFunction):
            extra_kwargs = dict()
    ...
    def visit_BoolOp(self, node: ast.BoolOp):
        ...
        if method_name is None:
            raise self._unsupported(node, "AST boolean operator '{}' is not (currently) implemented.".format(node.op.__name__))
        nontrivial_values = []
        for subnode in node.values:
            ...
            if not _is_triton_tensor(value):
                ...
                if value.type.is_block():
                    ...
                    warnings.warn_explicit("Logical operators 'and' and 'or' are deprecated for non-scalar tensors; please use '&' or '|' instead", category=UserWarning, filename=self.file_name, lineno=lineno, source=ast.unparse(node))
        ...
        for i, value in enumerate(values):
            if isinstance(value, ast.Constant):
                ...
                if not _is_constexpr(evaluated):
                    raise self._unsupported(node, 'Cannot evaluate f-string containing non-constexpr conversion values, found conversion of type ' + str(type(evaluated)))
    ...
    statically_implemented_functions: Dict[object, Callable[[ast.Call], Any]] = {language.core.static_assert: execute_static_assert, language.core.static_print: static_executor(print), ttgl.static_assert: execute_static_assert, ttgl.static_print: static_executor(print), int: static_executor(int), len: static_executor(len)}
...
def ast_to_ttir(fn, src, context, options, codegen_fns, module_map, module=None):
    ...
    generator = CodeGenerator(context, prototype, gscope=fn.get_capture_scope(), function_name=fn.repr(proxy), jit_fn=fn, is_kernel=True, file_name=file_name, begin_line=begin_line, options=options, codegen_fns=codegen_fns, module_map=module_map, module=module, is_gluon=fn.is_gluon())
    generator.visit(fn.parse())
