...
class BackendInstaller:
    ...
    def prepare(backend_name: str, backend_src_dir: str=None, is_external: bool=False):
        ...
        install_dir = os.path.join(os.path.dirname(__file__), 'python', 'teraxlang_triton', 'backends', backend_name)
        return Backend(name=backend_name, src_dir=backend_src_dir, backend_dir=backend_path, language_dir=language_dir, tools_dir=tools_dir, install_dir=install_dir, is_external=is_external)
...
def get_llvm_package_info():
    ...
    if (env_system_suffix := os.environ.get('TRITON_LLVM_SYSTEM_SUFFIX', None)):
        ...
        if arch == 'arm64' and is_linux_os('almalinux'):
            ...
            print(f'LLVM pre-compiled image is not available for {system}-{arch}. Proceeding with user-configured LLVM from source build.')
...
def download_and_copy(name, src_func, dst_path, variable, version, url_func):
    ...
    if os.path.exists(dst_path) and system == 'Linux' and (shutil.which(dst_path) is not None):
        ...
        curr_version = re.search('V([.|\\d]+)', curr_version)
...
class CMakeBuild(build_ext):
    ...
    def run(self):
        ...
        match = re.search('version\\s*(?P<major>\\d+)\\.(?P<minor>\\d+)([\\d.]+)?', out.decode())
    ...
    def build_extension(self, ext):
        ...
        cmake_args = ['-G', 'Ninja', '-DCMAKE_MAKE_PROGRAM=' + ninja_dir, '-DCMAKE_EXPORT_COMPILE_COMMANDS=ON', '-DLLVM_ENABLE_WERROR=ON', '-DCMAKE_LIBRARY_OUTPUT_DIRECTORY=' + extdir, '-DTRITON_BUILD_PYTHON_MODULE=ON', '-DPython3_EXECUTABLE:FILEPATH=' + sys.executable, '-DPython3_INCLUDE_DIR=' + python_include_dir, '-DTRITON_CODEGEN_BACKENDS=' + ';'.join([b.name for b in backends if not b.is_external]), '-DTRITON_PLUGIN_DIRS=' + ';'.join([b.src_dir for b in backends if b.is_external]), '-DTRITON_WHEEL_DIR=' + wheeldir]
        ...
        if check_env_flag('TRITON_BUILD_WITH_CLANG_LLD'):
            cmake_args += ['-DCMAKE_C_COMPILER=clang', '-DCMAKE_CXX_COMPILER=clang++', '-DCMAKE_LINKER=lld', '-DCMAKE_EXE_LINKER_FLAGS=-fuse-ld=lld', '-DCMAKE_MODULE_LINKER_FLAGS=-fuse-ld=lld', '-DCMAKE_SHARED_LINKER_FLAGS=-fuse-ld=lld']
        if check_env_flag('TRITON_BUILD_WITH_ASAN'):
            cmake_args += ['-DCMAKE_C_FLAGS=-fsanitize=address', '-DCMAKE_CXX_FLAGS=-fsanitize=address']
        passthrough_args = ['TRITON_BUILD_PROTON', 'TRITON_BUILD_WITH_CCACHE', 'TRITON_PARALLEL_LINK_JOBS']
...
def download_and_copy_dependencies():
    ...
    download_and_copy(name='nvcc', src_func=lambda system, arch, version: f'cuda_nvcc-{system}-{arch}-{version}-archive/bin/ptxas{exe_extension}', dst_path='bin/ptxas', variable='TRITON_PTXAS_PATH', version=NVIDIA_TOOLCHAIN_VERSION['ptxas'], url_func=lambda system, arch, version: f'https://developer.download.nvidia.com/compute/cuda/redist/cuda_nvcc/{system}-{arch}/cuda_nvcc-{system}-{arch}-{version}-archive.tar.xz')
    download_and_copy(name='cuobjdump', src_func=lambda system, arch, version: f'cuda_cuobjdump-{system}-{arch}-{version}-archive/bin/cuobjdump{exe_extension}', dst_path='bin/cuobjdump', variable='TRITON_CUOBJDUMP_PATH', version=NVIDIA_TOOLCHAIN_VERSION['cuobjdump'], url_func=lambda system, arch, version: f'https://developer.download.nvidia.com/compute/cuda/redist/cuda_cuobjdump/{system}-{arch}/cuda_cuobjdump-{system}-{arch}-{version}-archive.tar.xz')
    download_and_copy(name='nvdisasm', src_func=lambda system, arch, version: f'cuda_nvdisasm-{system}-{arch}-{version}-archive/bin/nvdisasm{exe_extension}', dst_path='bin/nvdisasm', variable='TRITON_NVDISASM_PATH', version=NVIDIA_TOOLCHAIN_VERSION['nvdisasm'], url_func=lambda system, arch, version: f'https://developer.download.nvidia.com/compute/cuda/redist/cuda_nvdisasm/{system}-{arch}/cuda_nvdisasm-{system}-{arch}-{version}-archive.tar.xz')
    download_and_copy(name='nvcc', src_func=lambda system, arch, version: f'cuda_nvcc-{system}-{arch}-{version}-archive/include', dst_path='include', variable='TRITON_CUDACRT_PATH', version=NVIDIA_TOOLCHAIN_VERSION['cudacrt'], url_func=lambda system, arch, version: f'https://developer.download.nvidia.com/compute/cuda/redist/cuda_nvcc/{system}-{arch}/cuda_nvcc-{system}-{arch}-{version}-archive.tar.xz')
    download_and_copy(name='cudart', src_func=lambda system, arch, version: f'cuda_cudart-{system}-{arch}-{version}-archive/include', dst_path='include', variable='TRITON_CUDART_PATH', version=NVIDIA_TOOLCHAIN_VERSION['cudart'], url_func=lambda system, arch, version: f'https://developer.download.nvidia.com/compute/cuda/redist/cuda_cudart/{system}-{arch}/cuda_cudart-{system}-{arch}-{version}-archive.tar.xz')
    download_and_copy(name='cupti', src_func=lambda system, arch, version: f'cuda_cupti-{system}-{arch}-{version}-archive/include', dst_path='include', variable='TRITON_CUPTI_INCLUDE_PATH', version=NVIDIA_TOOLCHAIN_VERSION['cupti'], url_func=lambda system, arch, version: f'https://developer.download.nvidia.com/compute/cuda/redist/cuda_cupti/{system}-{arch}/cuda_cupti-{system}-{arch}-{version}-archive.tar.xz')
    download_and_copy(name='cupti', src_func=lambda system, arch, version: f'cuda_cupti-{system}-{arch}-{version}-archive/lib', dst_path='lib/cupti', variable='TRITON_CUPTI_LIB_PATH', version=NVIDIA_TOOLCHAIN_VERSION['cupti'], url_func=lambda system, arch, version: f'https://developer.download.nvidia.com/compute/cuda/redist/cuda_cupti/{system}-{arch}/cuda_cupti-{system}-{arch}-{version}-archive.tar.xz')
...
def get_package_dirs():
    ...
    for backend in backends:
        if backend.is_external:
            continue
        yield (f'teraxlang_triton.backends.{backend.name}', backend.backend_dir)
        if backend.language_dir:
            for x in os.listdir(backend.language_dir):
                yield (f'teraxlang_triton.language.extra.{x}', os.path.join(backend.language_dir, x))
        if backend.tools_dir:
            for x in os.listdir(backend.tools_dir):
                yield (f'teraxlang_triton.tools.extra.{x}', os.path.join(backend.tools_dir, x))
    if check_env_flag('TRITON_BUILD_PROTON', 'ON'):
        yield ('teraxlang_triton.profiler', 'third_party/proton/proton')
        yield ('teraxlang_triton.profiler.hooks', 'third_party/proton/proton/hooks')

def get_packages():
    yield from find_packages(where='python')
    for backend in backends:
        yield f'teraxlang_triton.backends.{backend.name}'
        if backend.language_dir:
            for x in os.listdir(backend.language_dir):
                yield f'teraxlang_triton.language.extra.{x}'
        if backend.tools_dir:
            for x in os.listdir(backend.tools_dir):
                yield f'teraxlang_triton.tools.extra.{x}'
    if check_env_flag('TRITON_BUILD_PROTON', 'ON'):
        yield 'teraxlang_triton.profiler'
...
def add_link_to_backends(external_only):
    for backend in backends:
        ...
        if backend.language_dir:
            extra_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), 'python', 'teraxlang_triton', 'language', 'extra'))
            for x in os.listdir(backend.language_dir):
                src_dir = os.path.join(backend.language_dir, x)
                install_dir = os.path.join(extra_dir, x)
                update_symlink(install_dir, src_dir)
        if backend.tools_dir:
            extra_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), 'python', 'teraxlang_triton', 'tools', 'extra'))
            for x in os.listdir(backend.tools_dir):
...
def add_link_to_proton():
    ...
    proton_install_dir = os.path.join(os.path.dirname(__file__), 'python', 'teraxlang_triton', 'profiler')
...
def get_entry_points():
    entry_points = {}
    if check_env_flag('TRITON_BUILD_PROTON', 'ON'):
        entry_points['console_scripts'] = ['proton-viewer = teraxlang_triton.profiler.viewer:main', 'proton = teraxlang_triton.profiler.proton:main']
    entry_points['teraxlang_triton.backends'] = [f'{b.name} = teraxlang_triton.backends.{b.name}' for b in backends]
    return entry_points
...
BASE_CLASSIFIERS = ['Development Status :: 4 - Beta', 'Intended Audience :: Developers', 'Topic :: Software Development :: Build Tools', 'License :: OSI Approved :: MIT License']
PYTHON_CLASSIFIERS = [f'Programming Language :: Python :: {MIN_PYTHON[0]}.{m}' for m in range(MIN_PYTHON[1], MAX_PYTHON[1] + 1)]
CLASSIFIERS = BASE_CLASSIFIERS + PYTHON_CLASSIFIERS
setup(name=os.environ.get('TRITON_WHEEL_NAME', 'teraxlang'), version=TRITON_VERSION, author='zhangzn710', author_email='zhangzn710@gmail.com', description='A language and compiler for custom Deep Learning operations', long_description='', install_requires=["importlib-metadata; python_version < '3.10'"], packages=list(get_packages()), package_dir=dict(get_package_dirs()), entry_points=get_entry_points(), include_package_data=True, ext_modules=[CMakeExtension('triton', 'teraxlang_triton/_C/')], cmdclass={'bdist_wheel': plugin_bdist_wheel, 'build_ext': CMakeBuild, 'build_py': CMakeBuildPy, 'clean': CMakeClean, 'develop': plugin_develop, 'editable_wheel': plugin_editable_wheel, 'egg_info': plugin_egg_info, 'install': plugin_install, 'sdist': plugin_sdist}, zip_safe=False, keywords=['Compiler', 'Deep Learning'], url='https://github.com/deciding/txl', python_requires=PYTHON_REQUIRES, classifiers=CLASSIFIERS, test_suite='tests', extras_require={'build': ['cmake>=3.20,<4.0', 'lit'], 'tests': ['autopep8', 'isort', 'numpy', 'pytest', 'pytest-forked', 'pytest-xdist', 'scipy>=1.7.1', 'llnl-hatchet'], 'tutorials': ['matplotlib', 'pandas', 'tabulate']})