...
__all__ = ['AutoLayout', 'BlockedLayout', 'SliceLayout', 'DistributedLinearLayout', 'DotOperandLayout', 'NVMMADistributedLayout', 'NVMMASharedLayout', 'SwizzledSharedLayout']
...
class BlockedLayout(DistributedLayout):
    ...
    def _to_ir(self, builder):
        return builder.get_blocked_layout(self.size_per_thread, self.threads_per_warp, self.warps_per_cta, self.order, self.ctas_per_cga, self.cta_split_num, self.cta_order)
...
class SliceLayout(DistributedLayout):
    ...
    def _to_ir(self, builder):
        return builder.get_slice_layout(self.dim, self.parent._to_ir(builder))
...
class DistributedLinearLayout(DistributedLayout):
    ...
    def _to_ir(self, builder):
        return builder.get_distributed_linear_layout(self.reg_bases, self.lane_bases, self.warp_bases, self.block_bases, self.shape)
...
class NVMMADistributedLayout(DistributedLayout):
    ...
    def _to_ir(self, builder):
        return builder.get_mma_layout(self.version, self.warps_per_cta, self.ctas_per_cga, self.cta_split_num, self.cta_order, self.instr_shape)
...
class NVMMASharedLayout(SharedLayout):
    ...
    def _to_ir(self, builder):
        return builder.get_nvmma_shared_layout(self.swizzle_byte_width, self.element_bitwidth, self.transposed, self.fp4_padded, self.ctas_per_cga, self.cta_split_num, self.cta_order)

    @staticmethod
    @constexpr_function
    def get_default_for(block_shape, dtype, transposed=False, fp4_padded=False, ctas_per_cga=None, cta_split_num=None, cta_order=None):
        ...
        if len(block_shape) < 2 or flatten_outer_dim < 8:
            swizzle_byte_width = 0
        return NVMMASharedLayout(swizzle_byte_width=swizzle_byte_width, element_bitwidth=dtype.primitive_bitwidth, rank=rank, transposed=transposed, fp4_padded=fp4_padded, ctas_per_cga=ctas_per_cga, cta_split_num=cta_split_num, cta_order=cta_order)
...
class SwizzledSharedLayout(SharedLayout):
    ...
    def _to_ir(self, builder):
        return builder.get_swizzled_shared_layout(self.vec, self.per_phase, self.max_phase, self.order, self.ctas_per_cga, self.cta_split_num, self.cta_order)
