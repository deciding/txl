...
class DependenciesFinder(ast.NodeVisitor):
    ...
    def __init__(self, name, globals, nonlocals, src) -> None:
        ...
        self.supported_python_builtins = {'float', 'getattr', 'int', 'isinstance', 'len', 'list', 'max', 'min', 'print', 'range'}
        self.supported_modules = {GLUON_MODULE, TRITON_MODULE, 'copy', 'math'}
    ...
    def _update_hash(self, func):
        ...
        for k in self.used_global_vals.keys() & func.used_global_vals.keys():
            ...
            if v1 != v2:
                raise RuntimeError(f'Global variable {var_name} has value {v1} when compiling {self.name}, but inner kernel {func.__name__} has conflicting value {v2} from when it was first compiled.  This is not allowed.')
...
class KernelParam:
    ...
    def __init__(self, num: int, param: inspect.Parameter, do_not_specialize: bool, do_not_specialize_on_alignment: bool):
        self.num = num
        self._param = param
...
def serialize_specialization_data(name, signature, constants, attrs, options, key):
    ...
    import json
    obj = {'name': name, 'signature': signature, 'constant_keys': [list(x) for x in constants.keys()], 'constant_vals': list(constants.values()), 'attrs_keys': [list(x) for x in attrs.keys()], 'attrs_vals': list(attrs.values()), 'options': options.__dict__, 'key': key}
...
class JITCallable:
    ...
    def __init__(self, fn):
        ...
        src = src[re.search('^def\\s+\\w+\\s*\\(', src, re.MULTILINE).start():]
        self._src = src
        self.hash = None
        self.used_global_vals: Dict[Tuple[str, int], Tuple[Any, Dict[str, Any]]] = {}
    ...
    def cache_key(self):
        with self._hash_lock:
            ...
            dependencies_finder = DependenciesFinder(name=self._fn_name, globals=self.__globals__, nonlocals=nonlocals, src=self.src)
            dependencies_finder.visit(self.parse())
            self.hash = dependencies_finder.ret + str(self.starting_line_number)
            self.used_global_vals = dict(sorted(dependencies_finder.used_global_vals.items()))
            from triton.language.core import constexpr
            self.hash += str([(name, val) for (name, _), (val, _) in self.used_global_vals.items() if isinstance(val, constexpr)])
    ...
    def _set_src(self):
        raise AttributeError("Cannot set attribute 'src' directly. Use '_unsafe_update_src()' and manually clear `.hash` of all callersinstead.")
...
class TXLJITFunction(JITFunction):
    ...
    def run(self, *args, grid, warmup, **kwargs):
        ...
        if kernel is None:
            options, signature, constexprs, attrs = self._pack_args(backend, kwargs, bound_args, specialization, options)
        ...
        for (name, _), (val, globals_dict) in self.used_global_vals.items():
            if (newVal := globals_dict.get(name, not_present)) != val:
                raise RuntimeError(f'Global variable {name} has changed since we compiled this kernel, from {val} to {newVal}')
        if not warmup:
            ...
            kernel.run(grid_0, grid_1, grid_2, stream, kernel.function, kernel.packed_metadata, launch_metadata, knobs.runtime.launch_enter_hook, knobs.runtime.launch_exit_hook, *bound_args.values())
        return kernel

    def __init__(self, fn, version=None, do_not_specialize=None, do_not_specialize_on_alignment=None, debug=None, noinline=None, repr=None, launch_metadata=None, diff_mode=None, diff_select=None, log_dir=None, src_file=None, use_txl=True):
    ...
    def _do_compile(self, key, signature, device, constexprs, options, attrs, warmup):
        ...
        if async_mode is not None:
            ...
            def async_compile():
                return self.compile(src, target=target, options=options.__dict__, _env_vars=env_vars, diff_mode=self.diff_mode, diff_select=self.diff_select, log_dir=self.log_dir, use_txl=self.use_txl, txl_options=self.txl_options)
            ...
            kernel = self.compile(src, target=target, options=options.__dict__, diff_mode=self.diff_mode, diff_select=self.diff_select, log_dir=self.log_dir, use_txl=self.use_txl, txl_options=self.txl_options)
            if self.diff_mode:
                exit()
            kernel_cache[key] = kernel
            self._call_hook(knobs.runtime.jit_post_compile_hook, key, signature, device, constexprs, options, [attrs], warmup)
...
@overload
def jit(*, version=None, repr: Optional[Callable]=None, launch_metadata: Optional[Callable]=None, do_not_specialize: Optional[Iterable[int | str]]=None, do_not_specialize_on_alignment: Optional[Iterable[int | str]]=None, debug: Optional[bool]=None, noinline: Optional[bool]=None) -> Callable[[T], JITFunction[T]]:
    ...

def jit(fn: Optional[T]=None, *, version=None, repr: Optional[Callable]=None, launch_metadata: Optional[Callable]=None, do_not_specialize: Optional[Iterable[int | str]]=None, do_not_specialize_on_alignment: Optional[Iterable[int | str]]=None, debug: Optional[bool]=None, noinline: Optional[bool]=None, diff_mode: Optional[str]=None, diff_select: Optional[int]=None, log_dir: Optional[str]=None, src_file: Optional[str]=None, use_txl: bool=True) -> Union[JITFunction[T], Callable[[T], JITFunction[T]]]:
    """
    ...
    def decorator(fn: T) -> JITFunction[T]:
        ...
        if knobs.runtime.interpret:
            ...
            return InterpretedFunction(fn, version=version, do_not_specialize=do_not_specialize, do_not_specialize_on_alignment=do_not_specialize_on_alignment, debug=debug, noinline=noinline, repr=repr, launch_metadata=launch_metadata)
        else:
            return TXLJITFunction(fn, version=version, do_not_specialize=do_not_specialize, do_not_specialize_on_alignment=do_not_specialize_on_alignment, debug=debug, noinline=noinline, repr=repr, launch_metadata=launch_metadata, diff_mode=diff_mode, diff_select=diff_select, log_dir=log_dir, src_file=src_file, use_txl=use_txl)
    if fn is not None:
        return decorator(fn)
    else:
        return decorator