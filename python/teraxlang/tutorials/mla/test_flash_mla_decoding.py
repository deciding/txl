...
def generate_test_data(t: TestParam) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, Optional[torch.Tensor], Optional[torch.Tensor]]:
    ...
    q_value = torch.randn(t.b, t.h_q, t.s_q, 512).clamp(-1.0, 1.0)
    q_pe = torch.randn(t.b, t.h_q, t.s_q, 64).clamp(-1.0, 1.0)
    q = torch.cat([q_value, q_pe], dim=-1).permute(0, 2, 1, 3).contiguous()
    block_table = torch.arange(t.b * max_seqlen_pad // t.block_size, dtype=torch.int32).view(t.b, max_seqlen_pad // t.block_size)
    k_value = (torch.randn(t.b, t.h_kv, max_seqlen_pad, 512) / 10).clamp(-1.0, 1.0)
    k_pe = (torch.randn(t.b, t.h_kv, max_seqlen_pad, 64) / 10).clamp(-1.0, 1.0)
    k = torch.cat([k_value, k_pe], dim=-1).permute(0, 2, 1, 3).contiguous()
    blocked_k = k.view(t.b * (max_seqlen_pad // t.block_size), t.block_size, t.h_kv, t.d)
    assert blocked_k.size(0) == block_table.numel()
    if t.topk is None:
        for i in range(t.b):
        ...
        return (cache_seqlens, q, block_table, blocked_k, None, None, q_value, k_value, q_pe, k_pe)
    else:
        ...
        return (cache_seqlens, q, block_table, blocked_k, abs_indices, indices_in_kvcache, q_value, k_value, q_pe, k_pe)

def reference_torch(cache_seqlens: torch.Tensor, block_table: torch.Tensor, q: torch.Tensor, blocked_k: torch.Tensor, dv: int, is_causal: bool, indices: Optional[torch.Tensor]=None) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    ...
    def scaled_dot_product_attention(batch_idx: int, query: torch.Tensor, kv: torch.Tensor, dv: int, is_causal, indices: Optional[torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        h_q = query.size(0)
        h_kv = kv.size(0)
        s_q = query.shape[-2]
        s_k = kv.shape[-2]
    ...
    for i in range(b):
        ...
        cur_out, cur_lse = scaled_dot_product_attention(i, q[i].transpose(0, 1), cur_kv.transpose(0, 1), dv, is_causal, indices[i] if indices is not None else None)
...
def test_flash_mla(t: TestParam):
    ...
    cache_seqlens, q, block_table, blocked_k, abs_indices, indices_in_kvcache, q_value, k_value, q_pe, k_pe = generate_test_data(t)
    if t.is_fp8:
    ...
    torch.cuda.synchronize()
    tile_scheduler_metadata, num_splits = flash_mla.get_mla_metadata(cache_seqlens, t.s_q * t.h_q // t.h_kv, t.h_kv, t.h_q, t.is_fp8, t.topk)
    torch.cuda.synchronize()

    def run_flash_mla():
        return flash_mla.flash_mla_with_kvcache(q, blocked_k if not t.is_fp8 else blocked_k_quantized, block_table, cache_seqlens, t.dv, tile_scheduler_metadata, num_splits, causal=t.is_causal, is_fp8_kvcache=t.is_fp8, indices=indices_in_kvcache)
    ...
    assert check_is_allclose('out', out_ans, out_ref, abs_tol=0.0008, rel_tol=2.01 / 128, cos_diff_tol=5e-06)
    assert check_is_allclose('lse', lse_ans, lse_ref, abs_tol=1e-06, rel_tol=8.01 / 65536)
    assert check_is_allclose('txl_out', txl_ans_out, out_ref, abs_tol=0.0008, rel_tol=2.01 / 128, cos_diff_tol=5e-06)
    print('Correctness check passed!')
    print('===============================')
    print('Running performance test...')
    if t.test_performance:
        torch.cuda.synchronize()
        time_usage: float = triton.testing.do_bench(run_flash_mla) / 1000
        mean_attended_seqlens = cache_seqlens.float().mean().item() if t.topk is None else t.topk
        compute_volume_flop = t.b * t.h_q * t.s_q * sum([2 * t.d * mean_attended_seqlens, 2 * mean_attended_seqlens * t.dv])
        q_elem_size = torch.bfloat16.itemsize
        kv_token_size = 656 if t.is_fp8 else t.d * torch.bfloat16.itemsize
        memory_volume_B = t.b * sum([t.s_q * t.h_q * (t.d * q_elem_size), (t.s_q if t.topk is not None else 1) * mean_attended_seqlens * t.h_kv * kv_token_size, t.s_q * t.h_q * (t.dv * q_elem_size)])
        achieved_tflops = compute_volume_flop / time_usage / 1000000000000.0
        achieved_gBps = memory_volume_B / time_usage / 1000000000.0
        torch.cuda.synchronize()
        time_usage_txl: float = triton.testing.do_bench(runner_txl) / 1000
        achieved_tflops_txl = compute_volume_flop / time_usage_txl / 1000000000000.0
        achieved_gBps_txl = memory_volume_B / time_usage_txl / 1000000000.0
        print(f'FLASH MLA: {time_usage * 1000:.3f} ms, {achieved_tflops:.0f} TFLOPS, {achieved_gBps:.0f} GB/s')
        print(f'TXL MLA: {time_usage_txl * 1000:.3f} ms, {achieved_tflops_txl:.0f} TFLOPS, {achieved_gBps_txl:.0f} GB/s')

def main(torch_dtype, dump_dir=None):
    ...
    correctness_cases = [TestParam(b, s_q, s_k, is_varlen, is_causal, is_fp8, topk, test_performance=False) for b in [1, 2, 6, 64] for s_q in [1, 2, 4] for s_k in [20, 140, 4096] for is_varlen in [False, True] for is_causal in [False, True] for is_fp8, topk in [(False, None), (True, 128), (True, 2048)] if not (is_causal and topk is not None)]
    corner_cases = [TestParam(128, 2, 4096, is_varlen=True, is_causal=False, is_fp8=True, topk=topk, test_performance=False, is_all_indices_invalid=True) for topk in [128, 2048, 4096]] + [TestParam(128, 2, 4096, is_varlen=True, is_causal=is_causal, is_fp8=is_fp8, topk=topk, test_performance=False, have_zero_seqlen_k=True) for is_causal, is_fp8, topk in [(False, False, None), (True, False, None), (False, True, 128), (False, True, 2048)]]
    performance_cases = [TestParam(128, s_q, s_k, is_varlen=True, is_causal=is_causal, is_fp8=is_fp8, topk=topk, test_performance=True) for is_causal, is_fp8, topk in [(False, False, None), (True, False, None), (False, True, 2048)] for s_q in [1, 2] for s_k in [4096, 8192, 16384, 32768]]
    testcases = correctness_cases + corner_cases + performance_cases
    testcases = [TestParam(132, 1, s_k, is_varlen=False, is_causal=False, is_fp8=False, topk=None, test_performance=True) for s_k in [1024, 2048, 4096, 8192, 16384, 32768]]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dtype', type=str, choices=['bf16', 'fp16'], default='fp16', help='Data type to use for testing (bf16 or fp16)')
