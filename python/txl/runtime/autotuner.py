...
class Autotuner(KernelInterface):
    ...
    def _bench(self, *args, config, **meta):
        ...
        def kernel_call():
            ...
            _cpu_pre_hook = None
            if "_cpu_pre_hook" in current:
                current["_cpu_pre_hook"](full_nargs)
                _cpu_pre_hook = current["_cpu_pre_hook"]
                del current["_cpu_pre_hook"]
            ...
            try:
                ...
                try:
                    ...
                    raise
            if _cpu_pre_hook:
                current["_cpu_pre_hook"] = _cpu_pre_hook
    ...
    def run(self, *args, **kwargs):
        ...
        if config.pre_hook is not None:
            ...
            config.pre_hook(full_nargs)
        if "_cpu_pre_hook" in kwargs:
            full_nargs = {**self.nargs, **kwargs, **config.all_kwargs()}
            kwargs["_cpu_pre_hook"](full_nargs)
            del kwargs["_cpu_pre_hook"]
