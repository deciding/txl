...
#@txl.autotune(configs=list(filter(keep_tma, configs_tma)), key=["N_CTX", "HEAD_DIM", "FP8_OUTPUT"])
@txl.jit
def _attn_fwd_tma(sm_scale, M,  #
    ...
    #q_tma = txl.subtma(desc_q, [qo_offset_y,0], Q_layout, [BLOCK_M, HEAD_DIM], dtype)
        ...
        if USE_TMA and supports_tma() and not (torch.cuda.get_device_capability()[0] == 9
            ...
            if False:
                ...
                def _cpu_pre_hook(nargs):
                    hook_q((nargs["BLOCK_M"], HEAD_DIM_K))
                    hook_k((nargs["BLOCK_N"], HEAD_DIM_K))
                    hook_v((nargs["BLOCK_N"], HEAD_DIM_K))
                    hook_o((nargs["BLOCK_M"], HEAD_DIM_K))
                    return
                if no_tune:
            ...
            #    order_map=txl.OrderType.RIGHT
            #)
            extra_kern_args.update({'BLOCK_M':64, 'BLOCK_N':32, 'num_stages': 3, 'num_warps':4})
