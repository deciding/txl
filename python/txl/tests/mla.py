...
def mla_decode_latent_sharedZ_ws_dim2_2K_txl_change( # cutedsl 这里要求KV_SEQ_LEN是BLOCK_N*2的整数倍，因为我这边还没有写奇数倍数的逻辑
    ...
    #bP1 = txl.smem_alloc([BLOCK_M, BLOCK_N], dtype=dtype, num_stages=1)
    #bAccL = txl.smem_alloc([BLOCK_M, D//2], dtype=dtype, num_stages=1)
    #bAccR = txl.smem_alloc([BLOCK_M, D//2], dtype=dtype, num_stages=1)
    ...
    bar_L1_ready  = txl.mbar_alloc(256, num_stages=1)
    ...
    ## SMEM NOREUSE
    #cur_AccL = txl.get_buffer(bAccL, 0)
    #cur_AccR = txl.get_buffer(bAccR, 0)
    ...
    if txl.is_warpgroup([1]):  # left consumer
        ...
        # m_i = txl.frag_smem_load(cur_Max, (64,), max_reg_layout)
        m_i += tl.math.log2(l_i)
        accL = accL / l_i[:, None] # TODO
        ...
        #txl.smem_store(cur_AccL, accL.to(dtype))
        #txl.tma_store(cur_AccL, desc_o_lat, [qo_off, 0])
        #desc_o_lat.store([qo_off, 0], accL.to(dtype))
...
if __name__ == "__main__":
    ...
    test_op(1, 1, 64, 1, 128*2, 128, 64, algo=7, no_tune=no_tune)
    #test_op(1, 1, 64, 1, 256, 128, 64, algo=7, no_tune=no_tune)

    #test_op(16, 32, 1024, 1, 1024, 128, 64, algo=7, no_tune=no_tune)
