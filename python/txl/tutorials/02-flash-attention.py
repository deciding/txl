...
tma_nows_best_config = {
    ...
    "NUM_STAGES": 3,
    "NUM_CONSUMERS": 1,
}  # stages 3, num warps 4
tma_ws_best_config = {
    "BLOCK_M": 128,
    "BLOCK_N": 128,
    "NUM_CONSUMERS": 2,
    "NUM_STAGES": 2,
}  # stages: 3, num warps: 4, num_warpgroups: 3
...
def _attn_fwd_tma_txl(
    ...
    # q = desc_q.load([qo_offset_y, 0])
    bQ = txl.smem_alloc(
        [BLOCK_M, HEAD_DIM], dtype=dtype
    )  # bQ has only 1 buffer for reuse only
...
def _attn_fwd_ws_tma_txl1(
    ...
    # q = desc_q.load([qo_offset_y, 0])
    bQ0 = txl.smem_alloc(
        [BLOCK_M // 2, HEAD_DIM], dtype=dtype
    )  # bQ has only 1 buffer for reuse only
    ...
    if txl.is_warpgroup([0]):
        ...
        for start_n in range(lo, hi, BLOCK_N):
            ...
            cur_mbar_QK1 = txl.get_buffer(
                cMbar_QK1, bufIdxW
            )  # wait for the same buffer
...
@txl.jit
# @txl.jit(src_file="dump/1220fa/EPQW5AIXL3AN2UZZNBADFUWB3VEMODKYGXVELFXKWSRC6ZJLEMXA/_attn_fwd_ws_tma_txl4.ptx")
...
def _attn_fwd_ws_tma_txl4(
    ...
    if txl.is_warpgroup([1, 2]):
        ...
        # pl.enter_scope("scale1")
        # update acc
        p = p.to(dtype)
        # pl.exit_scope("scale1")
...
###################################################
# TXL + TMA + FA3 Algo2 Pingpong + Profiling
###################################################
...
###################################################
# OWN: TXL + TMA + FA3 Algo3 Pingpong + Intra Overlap + profile
###################################################
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, causal, sm_scale, algo=0, no_tune=False, profiling=False):
        ...
        if profiling:
            ...
            # proton.start("/workspace/dump/fa3", backend="instrumentation", mode='default:sampling_strategy=selective:sampling_options=0,4,8', data="trace")
        algo_map[algo][grid](
            sm_scale,
            M,  #
            q.shape[0],
...
def run_test(algo="hopper_txl_ws_fa3", dump_dir=None):
    import os
