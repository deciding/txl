...
# @txl.jit(diff_mode="ttgir", diff_select=5, log_dir="/workspace/dump/smem/")
@txl.jit
def txl_mla0(
    q_nope_desc,
    q_pe_desc,
    ...
    q_nope_buf = txl.smem_alloc([B_H, 512], dtype=tl.bfloat16, num_stages=1)
    sQnope = txl.get_buffer(q_nope_buf, 0)
    q_pe_buf = txl.smem_alloc([B_H, 64], dtype=tl.bfloat16, num_stages=1)
    sQpe = txl.get_buffer(q_pe_buf, 0)
    mbar_q_nope_buf = txl.mbar_alloc(1, num_stages=1)
    mbar_q_nope = txl.get_buffer(mbar_q_nope_buf, 0)
    mbar_q_pe_buf = txl.mbar_alloc(1, num_stages=1)
    mbar_q_pe = txl.get_buffer(mbar_q_pe_buf, 0)
    ...
    sM_buf = txl.smem_alloc([B_H], dtype=tl.float32, num_stages=1)
    sM = txl.get_buffer(sM_buf, 0)
    sL_buf = txl.smem_alloc([B_H], dtype=tl.float32, num_stages=2)
    sL0 = txl.get_buffer(sL_buf, 0)
    sL1 = txl.get_buffer(sL_buf, 1)
    ...
    mbar_k0_free0 = txl.get_buffer(mbar_k0_free, 0)
    mbar_k0_free1 = txl.get_buffer(mbar_k0_free, 1)
    mbar_k0_ready = txl.mbar_alloc(128, num_stages=2)
    mbar_k0_ready0 = txl.get_buffer(mbar_k0_ready, 0)
    mbar_k0_ready1 = txl.get_buffer(mbar_k0_ready, 1)
    mbar_k1_free = txl.mbar_alloc(128, num_stages=2)
    mbar_k1_free0 = txl.get_buffer(mbar_k1_free, 0)
    mbar_k1_free1 = txl.get_buffer(mbar_k1_free, 1)
    ...
    if txl.is_warpgroup([0, 1]):
        ...
        for block_idx in range(0, NUM_TOPK_BLOCKS, 2):
            ...
            if txl.is_warpgroup([0]):
                ...
                reg_is_kv_valid = txl.smem_load(
                    is_kv_valid0, layout_row
                )  # mind the shape
            else:
                # reg_is_kv_valid = txl.smem_load(is_kv_valid1, layout_acc) # mind the shape
                reg_is_kv_valid = txl.smem_load(
                    is_kv_valid1, layout_row
                )  # mind the shape
            rP = tl.where(reg_is_kv_valid, rP, float("-inf"))

            if txl.is_warpgroup([1]):
                txl.bar_wait(
                    wg0_bunch_0_ready, 256
                )  # wait for wg0 to provide half of sM

            # online_softmax_and_rescale_o
            # txl.mbar_wait(mbar_is_kv_valid_ready, cur_bar_wait_phase)
            cur_max: tl.tensor = tl.max(
                rP, axis=1
            )  # reduce on 2 rows, and a warp reduce
            cur_max *= SCALE_LOG2
            ...
            ):  # only store for every 4 because they are the same
                # txl.frag_smem_store(sM, new_maxs, layout_sM)
                txl.smem_store(
                    sM, new_maxs
                )  # TODO: remove the layout of frag_smem_store
            ...
            if txl.is_warpgroup([0]):
                ...
                txl.bar_arrive(
                    wg0_bunch_0_ready, 256
                )  # inform wg1 that half sM is saved
                rO = tl.dot(
                    rS, sV0l, rO
                )  # O0 += S0 @ V0l, [B_H, B_TOPK] x [B_TOPK, 256], use scaled rO and ready rS
                txl.dot_wait(0)
                txl.mbar_arrive(mbar_k0_free0)

                # 1.1. NamedBarriers, wait full sM from wg1
                txl.bar_wait(wg1_bunch_0_ready, 256)
                # new_rM = txl.smem_load(sM)
                new_rM = txl.smem_load(sM)
            ...
            if txl.is_warpgroup([1]):
                ...
                txl.bar_arrive(
                    wg1_bunch_0_ready, 256
                )  # inform wg0 of whole sM and let it rescale
                rO = tl.dot(
                    rS, sV1r, rO
                )  # O1 += S1 @ V1r, [B_H, B_TOPK] x [B_TOPK, 256], should not release k1 mbar, bcoz no dot wait
        ...
        if txl.is_warpgroup([0]):
            # peer_L = txl.smem_load(sL1)
            peer_L = txl.smem_load(sL1)
        else:
            # peer_L = txl.smem_load(sL0)
            peer_L = txl.smem_load(sL0)
        rL += peer_L
    ...
    if txl.is_warpgroup([2]):
        ...
        index_offs = (
            tl.arange(0, B_TOPK * GROUP_SIZE) // GROUP_SIZE
        )  # 4 strided with stride 128//8, e.g. 0 16 32 48

        inc_index_offs = tl.reshape(tl.arange(0, B_TOPK * 64) % 64, (64, 64))
        inc_index_offs = txl.relayout(
            inc_index_offs, (64, 64), index_layout2d
        )  # TODO: col 8x8, natually redundant or merged in reshape?
        ...
        for block_idx in range(0, NUM_TOPK_BLOCKS, 2):
            ...
            for buf_idx in tl.static_range(0, 2):
                ...
                cur_index = tl.load(
                    cur_g_indices + index_offs
                )  # 4 strided with stride 128//8
                cur_index_nope0 = cur_index * STRIDE_KV_NOPE_0  # 4 strided with 128
                cur_index_pe0 = cur_index * STRIDE_KV_PE_0
                cur_index_nope0 = tl.broadcast_to(
                    cur_index_nope0[:, None], (512, 8)
                )  # 4x8 strided with 128 rows
                cur_index_nope0 = tl.reshape(
                    cur_index_nope0, (64, 64)
                )  # make it col 8x8
                ...
                is_token_valid0 = tl.broadcast_to(
                    is_token_valid[:, None], (512, 8)
                )  # each thread repeat 8 times
                is_token_valid0 = tl.reshape(
                    is_token_valid0, (64, 64)
                )  # reshape auto makes it correct
