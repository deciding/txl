...
def generate_test_data(t: TestParam) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, Optional[torch.Tensor], Optional[torch.Tensor]]:
    ...
    max_seqlen_pad = cdiv(max_seqlen, 256) * 256 #256
    cache_seqlens = cache_seqlens_cpu.cuda() # [132] each value is 256

    q_value = torch.randn(t.b, t.h_q, t.s_q, 512).clamp(-1.0, 1.0)
    q_pe = torch.randn(t.b, t.h_q, t.s_q, 64).clamp(-1.0, 1.0)
    q = torch.cat([q_value, q_pe], dim=-1).permute(0, 2, 1, 3).contiguous()  # [132, 64, 128, 576]
    # q = torch.randn(t.b, t.s_q, t.h_q, t.d) # [132, 64, 128, 576]
    # q.clamp_(min=-1.0, max=1.0)

    block_table = torch.arange(t.b * max_seqlen_pad // t.block_size, dtype=torch.int32).view(t.b, max_seqlen_pad // t.block_size) # [132, 4]
    ...
    k = torch.cat([k_value, k_pe], dim=-1).permute(0, 2, 1, 3).contiguous()  # [132, max_seqlen_pad, 1, 576]
    blocked_k = k.view(t.b * (max_seqlen_pad // t.block_size), t.block_size, t.h_kv, t.d)  # [?, block_size, h_kv, d]
...
def test_flash_mla(t: TestParam):
    ...
    # q_txl = torch.randn((t.b, t.h_q, t.s_q, 512), dtype=torch.float16, device='cuda').clamp(-1.0, 1.0)
    # q_pe_txl = (torch.randn((t.b, t.h_q, t.s_q, 64), dtype=torch.float16, device='cuda') / 10).clamp(-1.0, 1.0)
    # kv_txl = torch.randn((t.b, t.h_kv, t.s_k, 512), dtype=torch.float16, device='cuda').clamp(-1.0, 1.0)
    # kv_pe_txl = (torch.randn((t.b, t.h_kv, t.s_k, 64), dtype=torch.float16, device='cuda') / 10).clamp(-1.0, 1.0)
...
def main(torch_dtype):
    ...
    testcases = [
        ...
        TestParam(132, 1, s_k, is_varlen=False, is_causal=False, is_fp8=False, topk=None, test_performance=True)
        for s_k in [1024, 2048, 4096, 8192, 16384, 32768]
    ]
