...
def generate_test_data(
    ...
    q = (
        torch.cat([q_value, q_pe], dim=-1).permute(0, 2, 1, 3).contiguous()
    )  # [132, 64, 128, 576]
    ...
    ).view(t.b, max_seqlen_pad // t.block_size)  # [132, 4]
    ...
    k = (
        torch.cat([k_value, k_pe], dim=-1).permute(0, 2, 1, 3).contiguous()
    )  # [132, max_seqlen_pad, 1, 576]
    blocked_k = k.view(
        t.b * (max_seqlen_pad // t.block_size), t.block_size, t.h_kv, t.d
    )  # [?, block_size, h_kv, d]
...
def test_flash_mla(t: TestParam):
    ...
    runner_txl = flash_mla.make_mla_runner_txl(
        q_value, k_value, q_pe, k_pe, 1 / math.sqrt(576), algo=0
    )
    ...
    assert check_is_allclose(
        "out", out_ans, out_ref, abs_tol=8e-4, rel_tol=2.01 / 128, cos_diff_tol=5e-6
    )
    assert check_is_allclose(
        "lse", lse_ans, lse_ref, abs_tol=1e-6, rel_tol=8.01 / 65536
    )
    assert check_is_allclose(
        "txl_out",
        txl_ans_out,
    ...
    if t.test_performance:
        ...
        print(
            f"FLASH MLA: {time_usage * 1000:.3f} ms, {achieved_tflops:.0f} TFLOPS, {achieved_gBps:.0f} GB/s"
        )
        print(
            f"TXL MLA: {time_usage_txl * 1000:.3f} ms, {achieved_tflops_txl:.0f} TFLOPS, {achieved_gBps_txl:.0f} GB/s"
        )
